...
- **MAC 破坏 R7**：`exec_inst` 将 `self.regs[7]` 写为溢出标志，因此不能在 MAC 循环中用 R7 做指针；改用 R5 规避

### v0.7 新增文件

- `examples/tnn.tri` — 4→4→1 TNN 汇编推理
- `examples/tnn_ref.py` — Python 参考实现 + 8 个测试向量
- `AGENTS.md` — AI 知识库
- `opencode.json`, `.github/workflows/ci.yml`, `.github/ISSUE_TEMPLATE/` — 基础设施
...
### D1 工具链完善
- **反汇编器**：`.tribin` → 可读汇编，辅助调试
- **调试器**：断点、单步、寄存器查看、跟踪执行
- **验收**：能用调试器定位一个已知 bug

### D2 内存模型统一
- **现状**：VM 初始布局固定 (256/20000/40000/55000)，但汇编器输出连续从 0 排放
- **方案**：加链接器/加载器阶段，将汇编器输出重定位到 VM 布局
- **验收**：汇编程序可使用完整的 59049 tryte 空间

### D3 高阶语言 / DSL
- **目标**：设计一个极简 C-like 语言，编译到 TriASM
- **验收**：能用 DSL 编写 Fibonacci 程序，输出结果一致

### D4 VM 扩展
- **扩指令**：移位、位操作、NAND 等
- **扩系统调用**：文件 I/O、随机数、时间
- **解寻址限制**：LDI 支持 ±364 以外的地址（通过多指令序列或新寻址模式）
- **验收**：扩展后仍能运行原有示例

### D5 形式化验证
- **属性测试**：用 proptest 验证算术正确性、编解码往返、溢出语义完备性
- **验收**：1000+ 随机属性测试覆盖核心数学层 (Trit/Tryte)
